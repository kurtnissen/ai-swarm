...
export async function invokeGeminiAsync(
    ...
    try {
        ...
        // =======================================================================
        // STEP 2: Spawn Gemini CLI in detached process group
        // =======================================================================
        ...
        const geminiProcess = spawn('gemini', args, {
            ...
            // Create new process group for proper cleanup of Gemini + all child processes
            detached: true,
        });
        ...
        // Track PID for logging and process group cleanup
        const childPid = geminiProcess.pid;
        logger.debug({ pid: childPid }, 'Spawned Gemini CLI process (detached process group)');
        ...
        const killChild = () => {
            ...
            logger.warn({ pid: childPid }, 'Killing Gemini CLI process group');
            try {
                // Kill the entire process group (Gemini + any spawned children)
                // Negative PID means kill the entire process group
                process.kill(-childPid, 'SIGKILL');
            } catch (killErr: any) {
                // ESRCH means process already dead - that's fine
                if (killErr.code !== 'ESRCH') {
                    logger.debug({ err: killErr }, 'Error killing Gemini process group (may already be dead)');
                }
            }
        };
        ...
        await new Promise<void>((resolve, reject) => {
            ...
            const timeoutId = setTimeout(() => {
                if (!resolved) {
                    resolved = true;
                    logger.error({ pid: childPid, timeoutMs: timeout }, 'Gemini CLI timed out');
                    killChild();
            ...
            geminiProcess.on('close', (code) => {
                if (!resolved) {
                    ...
                    logger.debug({ pid: childPid, exitCode: code }, 'Gemini CLI process exited');
                    if (code === 0) {
                        resolve();
                    } else {
            ...
            geminiProcess.on('error', (error) => {
                if (!resolved) {
                    resolved = true;
                    clearTimeout(timeoutId);
                    logger.error({ err: error, pid: childPid }, 'Gemini CLI process error');
                    killChild();
