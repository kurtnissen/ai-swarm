...
### Added
- **Health Guardian:** New monitoring service that runs in Portal and can automatically shutdown workers if dangerous conditions are detected (memory > 90%, load > 100, process leaks)
- **Health API:** New `/api/health` endpoint to check system health status

### Fixed
- **CRITICAL: Heartbeat Auth Check Process Leak:** Replaced `claude doctor` command (spawned every 30s per worker) with file-based credential checks - this was the root cause of server crashes due to orphaned processes accumulating
- **Infrastructure:** Added `restart: unless-stopped` to temporal, temporal-ui, postgres, and redis services to ensure auto-recovery after server reboots
- **Gemini CLI Process Leak:** Applied same orphaned process fix to Gemini CLI that was applied to Claude CLI - uses detached process groups and `process.kill(-pid, 'SIGKILL')` for proper cleanup
- Claude CLI processes now have 10-minute timeout and proper cleanup to prevent zombie process accumulation
- **Deployment:** Updated `docker-compose.yml` default `IMAGE_TAG` to `3.0.1` to correctly pull the patched worker image (previously defaulted to 3.0.0).
